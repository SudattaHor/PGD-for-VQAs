...
class MVC:
    ...
    def __init__(self, edges, nlayers=2, max_iterations=100, step_size=0.01,
        ...
        def probability_circuit(gamma, alpha):
            for i in range(nqubits):
                qml.Hadamard(wires=i)
            qml.layer(qaoa_layer, nlayers, gamma, alpha)
            return qml.probs(wires=range(nqubits))
        self.probability_circuit = probability_circuit
    ...
    def plot_prob(self, params, *args, **kwargs):
        probs = self.probability_circuit(params[0], params[1])
        plt.style.use("seaborn")
        plt.bar(range(2 ** self.nqubits), probs)
        plt.show()
